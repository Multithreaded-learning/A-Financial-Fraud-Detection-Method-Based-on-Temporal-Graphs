...
int main(){
    ...
    ifstream infile;
    ifstream infliree;
    infliree.open("D:\\clionfile\\40000s.txt");
    if(!infile){
        cout<<"无法打开文件"<<endl;
        exit(1);
    }
    ...
    graph g(N,M,T);//创建节点图  T为 线程数
    int a,b,c,d;
//    while(infliree>>a>>b>>c>>d){//有金额使用这个
//        g.makescc(a,b,c,d);
    while(infliree>>a>>b>>c){
        g.makescc(a,b,c);
    }
    ...
    JMMTcyc_run_time=(time_over3.QuadPart-time_start3.QuadPart)/dqFreq3;
